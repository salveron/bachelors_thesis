...
\end{equation}
where $b=1.019$ and the ERB function was the same as in equation \ref{equation:ERB_1990} for frequency in Hertz.\\

Then a unit-step function was applied to the output along with a cubic root function that helped to better emphasize low amplitudes in the cochleagram. Finally, the output was passed through a filterbank of bandpass Butterworth filters to limit the frequency response of the gammatone filters. For each gammatone filter the bandwidths were different. They were uniformly distributed on a linear scale between 50\,Hz for the filter with the lowest center frequency and 1\,kHz for the highest frequency filter. The resulting cochleagrams for C-major and A-major scales are shown on figure \ref{img:cochleagram_example}.\\
